...
for(a in 1:length(Thresholds)){
  ...
  TopModelDetectionDF_single$Predictedsignal <-
    ifelse(TopModelDetectionDF_single$probability  <=Thresholds[a], 'noise','crestedargus')
  ...
  caretConf <- caret::confusionMatrix(
    ...
    as.factor(TopModelDetectionDF_single$signal),positive = 'crestedargus',
    mode = 'everything')
